...
function Entrance_Payment() {
  const [loading, setLoading] = useState(false);
  const [enrollmentFee, setEnrollmentFee] = useState(0);
  const [registrationFee, setRegistrationFee] = useState(0);
  const [totalAmount, setTotalAmount] = useState(0);
  const location = useLocation();
  const { courseType, courseName , id , name } = location.state;

// console.log(courseType , courseName , id)
  ...
  const fetchFeeInformation = () => {
    ...
    const apiUrl = `https://sssutms.ac.in/apitest/admin/EntranceFee2/?sessionYear=${sessionYear}&courseName=${courseName}`;
  ...
  const getData = (data) => {
    setLoading(false);
    return fetch(`https://sssutms.ac.in/apitest/entrancepayment`, {
      method: "POST",
      headers: {
...
export default Entrance_Payment;